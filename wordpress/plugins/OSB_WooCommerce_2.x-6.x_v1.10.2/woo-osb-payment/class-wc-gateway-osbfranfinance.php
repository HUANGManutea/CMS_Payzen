...
class WC_Gateway_OsbFranfinance extends WC_Gateway_OsbStd
{

    protected $osb_countries = array('FR', 'GP', 'MQ', 'GF', 'RE', 'YT'); // France and DOM.
    protected $osb_currencies = array('EUR');

    public function __construct()
    {
        $this->id = 'osbfranfinance';
        $this->icon = apply_filters('woocommerce_osbfranfinance_icon', WC_OSB_PLUGIN_URL . 'assets/images/franfinance.png');
        $this->has_fields = true;
        $this->method_title = self::GATEWAY_NAME. ' - ' . __('Franfinance payment', 'woo-osb-payment');
    ...
    public function init_form_fields()
    {
        ...
        $this->form_fields['enabled']['description'] = __('Enables / disables Franfinance payment.', 'woo-osb-payment');

        $this->form_fields['title']['default'] = __('Payment with Franfinance', 'woo-osb-payment');
        ...
        $this->form_fields['payment_options'] = array(
            'title' => __('FRANFINANCE PAYMENT OPTIONS', 'woo-osb-payment'),
            'type' => 'title'
        );
        ...
        $this->form_fields['payment_options'] = array(
            ...
            'columns' => $columns,
            'default' => array (
                '3x' => array('label' => sprintf(__('Payment in %s times', 'woo-osb-payment'), '3'), 'count' => '3', 'fees' => '-1', 'amount_min' => '100', 'amount_max' => '3000'),
                '4x' => array('label' => sprintf(__('Payment in %s times', 'woo-osb-payment'), '4'), 'count' => '4', 'fees' => '-1', 'amount_min' => '100', 'amount_max' => '4000'),
            ),
            ...
            function osbAddOption(fieldName, record, key) {
                ...
                jQuery.each(orderedRecord, function(attr, value) {
                    ...
                    switch (attr) {
                        ...
                        case 'fees':
                            optionLine += '<select style="width: ' + width + 'px;" name="' + inputName + '" id="' + inputName + '">';
                            optionLine += '<?php $options = array('-1' => sprintf(__('%s Back Office configuration', 'woo-osb-payment'), self::BACKOFFICE_NAME), 'N' => __('Without fees', 'woo-osb-payment'), 'Y' => __('With fees', 'woo-osb-payment'));
                                                foreach ($options as $key => $value) {
                                                    echo '<option value="' . $key . '">' . $value . '</option>';
                                                } ?>';
...
<?php
    ...
    public function validate_payment_options_field($key, $value = null)
    {
        ...
        foreach ($value as $code => $option) {
            ...
            if (! $option['label']) {
                $value[$code]['label'] = sprintf(__('Payment in %s times', 'woo-osb-payment'), $option['count']);
            }
        }
    ...
    public function payment_fields()
    {
        ...
        if (count($options) == 1) {
            ...
            echo '<li style="list-style-type: none;">
                    <input type="hidden" id="osbfranfinance_option_' . $key . '" value="' . $key . '" name="osbfranfinance_option">
            ...
            foreach ($options as $key => $option) {
                echo '<li style="list-style-type: none;">
                        <input class="radio" type="radio"'. ($first == true ? ' checked="checked"' : '') . ' id="osbfranfinance_option_' . $key . '" value="' . $key . '" name="osbfranfinance_option">
                        <label for="osbfranfinance_option_' . $key . '" style="display: inline;">' . $option['label'] . '</label>
                      </li>';
                $first = false;
            }
        }
    ...
    public function process_payment($order_id)
    {
        ...
        $options = $this->get_available_options();
        $option = $options[$_POST['osbfranfinance_option']];

        // Save selected payment option into session...
        set_transient('osbfranfinance_option_' . $order_id, $option);
    ...
    protected function osb_fill_request($order)
    {
        ...
        $this->osb_request->set('validation_mode', '0');
        $this->osb_request->set('capture_delay', '0');

        $option = get_transient('osbfranfinance_option_' . self::get_order_property($order, 'id'));
        $this->osb_request->set('payment_cards', 'FRANFINANCE_' . $option['count'] . 'X');

        if ($option['fees'] !== '-1') {
            $fees = $option['fees'] ? 'Y' : 'N';
            $this->osb_request->set('acquirer_transient_data', '{"FRANFINANCE":{"FEES_' . $option['count'] . 'X":"' . $fees . '"}}');
        }

        delete_transient('osbfranfinance_option_' . self::get_order_property($order, 'id'));
    }
}
