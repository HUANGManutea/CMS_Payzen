...
class OsbRestTools
{
    public static function convertRestResult($answer)
    {
        ...
        $response['vads_currency'] = OsbApi::getCurrencyNumCode(self::getProperty($transaction, 'currency'));
        ...
        if ($transactionDetails = self::getProperty($transaction, 'transactionDetails')) {
            ...
            $effectiveCurrency = OsbApi::getCurrencyNumCode(self::getProperty($transactionDetails, 'effectiveCurrency'));
    ...
    public static function checkResult($response, $expectedStatuses = array())
    {
        ...
        if ($response['status'] != 'SUCCESS') {
            ...
            require_once 'OsbRestException.php';
            throw new OsbRestException($errorMessage, $answer['errorCode']);
        } elseif (! empty($expectedStatuses) && ! in_array($answer['detailedStatus'], $expectedStatuses)) {
            throw new Exception(sprintf(__('Unexpected transaction type received (%s).', 'woo-osb-payment'), $answer['detailedStatus']));
        }
    }
}
