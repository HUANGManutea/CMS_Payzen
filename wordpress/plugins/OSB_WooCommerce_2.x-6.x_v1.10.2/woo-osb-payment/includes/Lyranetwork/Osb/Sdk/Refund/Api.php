...
namespace Lyranetwork\Osb\Sdk\Refund;
...
class Api {
     ...
     *
     * @param \Lyranetwork\Osb\Sdk\Refund\Processor $refundProcessor
     ...
     * Refund money.
     *
     * @param \Lyranetwork\Osb\Sdk\Refund\OrderInfo $orderInfo
     * @param float $amount
     */
    public function refund($orderInfo, $amount)
    {
        ...
        // Get currency.
        $currency = OsbApi::findCurrencyByAlphaCode($orderInfo->getOrderCurrencyIsoCode());
    ...
    {
        /**
         * @var Lyranetwork\Osb\Sdk\Rest\Api $client
         * */
        $client = new OsbRest(
            $this->restServerUrl,
            $this->siteId,
    ...
    private function checkRestResult($response, $expectedStatuses = array())
    {
        ...
        if ($response['status'] !== 'SUCCESS') {
            ...
            throw new \Lyranetwork\Osb\Sdk\Refund\WsException($errorMessage, $answer['errorCode']);
    ...
    private function refundFromOneTransaction($orderInfo, $amountInCents, $transaction, $currency)
    {
        ...
        $successStatuses = array_merge(
            OsbApi::getSuccessStatuses(),
            OsbApi::getPendingStatuses()
        );
