...
class WC_Gateway_OsbRegroupedOther extends WC_Gateway_OsbStd
{
    public function __construct()
    {
        $this->id = 'osbregroupedother';
        $this->icon = apply_filters('woocommerce_' . $this->id . '_icon', WC_OSB_PLUGIN_URL . 'assets/images/other.png');
        $this->has_fields = true;
        $this->method_title = self::GATEWAY_NAME . ' - ' . __('Other payment means', 'woo-osb-payment');
    ...
    public function init_form_fields()
    {
        ...
        $this->form_fields['enabled']['description'] = __('Enables / disables this payment method.', 'woo-osb-payment');

        $this->form_fields['title']['default'] = __('Other payment means', 'woo-osb-payment');
        ...
        if (version_compare($woocommerce->version, '2.3.0', '>=')) {
            $this->form_fields['regroup_enabled'] = array(
                'title' => __('Regroup payment means', 'woo-osb-payment'),
                'label' => __('Enable / disable', 'woo-osb-payment'),
                'type' => 'checkbox',
                'default' => 'no',
                'description' => __('If this option is enabled, all the payment means added in this section will be displayed within the same payment submodule.', 'woo-osb-payment')
            );
        }
        ...
        $columns['payment_mean'] = array(
            'title' => __('Means of payment', 'woo-osb-payment'),
            'width' => '250px'
        );
        ...
        $columns['countries'] = array(
            'title' => __('Countries', 'woo-osb-payment'),
            'width' => '175px',
        );
        ...
        $columns['send_cart_data'] = array(
            'title' => __('Cart data', 'woo-osb-payment'),
            'width' => '92px',
        );

        $this->form_fields['payment_means'] = array(
            'title' => __('Payment means', 'woo-osb-payment'),
            'type' => 'table',
            'columns' => $columns,
            'description' => $descr
        );
        ...
        $this->form_fields['extra_payment_means'] = array(
            'title' => __('Add payment means', 'woo-osb-payment'),
            'type' => 'table',
            'columns' => $culumns,
            'description' => $descr
        );
    ...
    {
        return __('Method title to display on payment means page. Used only if &laquo; Regroup payment means &raquo; option is enabled.', 'woo-osb-payment');
    }

    protected function get_method_description_field_description()
    {
        return __('This controls the description which the user sees during checkout. Used only if &laquo; Regroup payment means &raquo; option is enabled.', 'woo-osb-payment');
    }
    ...
    public function osb_admin_head_script()
    {
        ...
        //<!--
                    ...
                    switch (attr) {
                        ...
                        case 'countries':
                            optionLine += '<div><select style="display:none; width: ' + width + 'px; height: 150px; background-color: white; padding: 2px;" name="' + inputName + '[]" id="' + inputName +
                                         '" multiple="multiple" onblur="javascript:osbDisplayMultiSelect(\'' + inputName + '\'); osbDisplayLabel(\'' + inputName + '\');">';
                            optionLine += '<?php
                                                ...
                                                if (labelValue == '') {
                                                    labelValue = '<?php echo __('Click to add countries.', 'woo-osb-payment')?>';
                                                }

                            optionLine += '</select><label style="width:100%;" id="label_' + inputName + '" onclick="javascript:osbDisplayMultiSelect(\'' + inputName + '\');" >' + labelValue + '</label></div>';
                            break;
                        ...
                        case 'send_cart_data':
                            optionLine += '<select style="width: ' + width + 'px;" name="' + inputName + '" id="' + inputName + '">';
                            optionLine += '<?php $options = array('n' => __('No', 'woo-osb-payment'), 'y' => __('Yes', 'woo-osb-payment'));
            ...
            function osbDisplayMultiSelect(selectId) {
            ...
            function osbDisplayLabel(selectId) {
                var select = document.getElementById(selectId);
                var label = document.getElementById('label_' + selectId);
            ...
            function getLabelText(select) {
                ...
                if (! labelText) {
                    labelText = '<?php echo __('Click to add countries.', 'woo-osb-payment')?>';
                }
...
<?php
            ...
            if (($option['amount_min'] && (! is_numeric($option['amount_min']) || $option['amount_min'] < 0))
                ...
                if (in_array($option['payment_mean'], $used_cards)) {
                    ...
                    if (! $option['label']) {
                        ...
                        $value[$code]['label'] = sprintf(__('Payment with %s', 'woo-osb-payment'), $cards[$option['payment_mean']]);
                    }
    ...
    public function validate_extra_payment_means_field($key, $value = null)
    {
        $name = $this->plugin_id . $this->id . '_' . $key;
        $value = $value ? $value : (key_exists($name, $_POST) ? $_POST[$name] : array());
        $used_cards = array_keys(OsbApi::getSupportedCardTypes());
    ...
    protected function osb_fill_request($order)
    {
        ...
        if (isset($option['capture_delay']) && is_numeric($option['capture_delay'])) {
            $this->osb_request->set('capture_delay', $option['capture_delay']);
        }

        if (isset($option['validation_mode']) && ($option['validation_mode'] !== '-1')) {
            $this->osb_request->set('validation_mode', $option['validation_mode']);
        }
