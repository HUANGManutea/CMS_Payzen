...
class WC_Gateway_OsbOther extends WC_Gateway_OsbStd
{
    ...
    public function __construct($payment_code, $payment_title)
    {
        ...
        $this->regrouped_other_payments = new WC_Gateway_OsbRegroupedOther();

        $code = strtolower($this->payment_code);
        $this->id = 'osbother_' . $code;
    ...
    public function get_title()
    {
        ...
        if (! $title) {
            ...
            $title = sprintf(__('Payment with %s', 'woo-osb-payment'), $cards[$this->payment_code]);
        }
    ...
    protected function osb_fill_request($order)
    {
        ...
        // Set payment card.
        $this->osb_request->set('payment_cards', $this->payment_code);
