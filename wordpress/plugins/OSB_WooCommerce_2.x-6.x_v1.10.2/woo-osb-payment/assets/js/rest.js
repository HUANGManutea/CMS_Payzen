...
var OSB_DFAULT_MESSAGES = ['CLIENT_300', 'CLIENT_304', 'CLIENT_502', 'PSP_539']; // Use default messages for these errors.
var OSB_EXPIRY_ERRORS = ['PSP_108', 'PSP_136', 'PSP_649'];

var osbInitRestEvents = function(KR) {
    KR.onError(function(e) {
        jQuery('#osbstd_rest_processing').css('display', 'none');
        jQuery('form.checkout').removeClass('processing').unblock();
        jQuery('#order_review').unblock();

        var msg = '';
        if (OSB_DFAULT_MESSAGES.indexOf(e.errorCode) > -1) {
            msg = e.errorMessage;
            var endsWithDot = (msg.lastIndexOf('.') == (msg.length - 1) && msg.lastIndexOf('.') >= 0);

            msg += (endsWithDot ? '' : '.');
        } else {
            msg = osbTranslate(e.errorCode);
        }

        // Expiration errors, display a link to refresh the page.
        if (OSB_EXPIRY_ERRORS.indexOf(e.errorCode) >= 0) {
            msg += ' <a href="#" onclick="window.location.reload(); return false;">'
                + osbTranslate('RELOAD_LINK') + '</a>';
        }

        jQuery('.kr-form-error').html('<span style="color: red;"><span>' + msg + '</span></span>');
        jQuery('ul.osbstd-view-bottom').show();
    });
...
var osbTranslate = function(code) {
    var lang = OSB_LANGUAGE; // Global variable that contains current language.
    var messages = OSB_ERROR_MESSAGES.hasOwnProperty(lang) ? OSB_ERROR_MESSAGES[lang] : OSB_ERROR_MESSAGES['en'];
