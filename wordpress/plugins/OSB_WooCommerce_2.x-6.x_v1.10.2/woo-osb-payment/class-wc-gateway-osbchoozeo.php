...
class WC_Gateway_OsbChoozeo extends WC_Gateway_OsbStd
{
    ...
    {
        $this->id = 'osbchoozeo';
        $this->icon = apply_filters('woocommerce_osbchoozeo_icon', WC_OSB_PLUGIN_URL . 'assets/images/choozeo.png');
        $this->has_fields = true;
        $this->method_title = self::GATEWAY_NAME. ' - ' . __('Choozeo payment', 'woo-osb-payment');
    ...
    public function init_form_fields()
    {
        ...
        $this->form_fields['enabled']['description'] = __('Enables / disables Choozeo payment.', 'woo-osb-payment');

        $this->form_fields['title']['default'] = __('Payment with Choozeo without fees', 'woo-osb-payment');
        ...
        $this->form_fields['multi_options'] = array(
            'title' => __('CHOOZEO PAYMENT OPTIONS', 'woo-osb-payment'),
            'type' => 'title'
        );
        ...
        $this->form_fields['payment_options'] = array(
            ...
            'description' => __('Define amount restriction for each card.', 'woo-osb-payment')
        );
    }
        ...
        foreach ($options as $key => $option) {
            ...
            if (count($options) == 1) {
                echo '<input type="hidden"
                             id="osbchoozeo_option_' . $key . '"
                             value="' . $key . '"
                             name="osbchoozeo_option">';
            } else {
            ...
            echo '<label for="osbchoozeo_option_' . $key . '" style="display: inline;">' . '
    ...
    public function process_payment($order_id)
    {
        ...
        $option = $_POST['osbchoozeo_option'];
        $options = $this->get_available_options();
        $label = $options[$option]['label'];

        // Save selected payment option into session...
        set_transient('osbchoozeo_option_' . $order_id, $option);
    ...
    protected function osb_fill_request($order)
    {
        ...
        $option = get_transient('osbchoozeo_option_' . self::get_order_property($order, 'id'));
        $this->osb_request->set('payment_cards', $option);
        ...
        $this->osb_request->set('cust_country', 'FR');

        delete_transient('osbchoozeo_option_' . self::get_order_property($order, 'id'));
    }
}
