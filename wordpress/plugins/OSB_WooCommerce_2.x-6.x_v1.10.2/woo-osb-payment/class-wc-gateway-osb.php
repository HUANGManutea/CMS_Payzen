...
use Lyranetwork\Osb\Sdk\Form\Request as OsbRequest;
use Lyranetwork\Osb\Sdk\Rest\Api as OsbRest;

class WC_Gateway_Osb extends WC_Payment_Gateway
{
    const GATEWAY_CODE = 'OSB';
    const GATEWAY_NAME = 'OSB';
    const BACKOFFICE_NAME = 'OSB';
    const GATEWAY_URL = 'https://secure.osb.pf/vads-payment/';
    const REST_URL = 'https://secure.osb.pf/api-payment/';
    const STATIC_URL = 'https://static.osb.pf/static/';
    const LOGO_URL = 'https://secure.osb.pf/static/latest/images/type-carte/';
    ...
    const SUPPORT_EMAIL = 'support@osb.pf';
    ...
    public function __construct()
    {
        ...
        $this->method_title = self::GATEWAY_NAME . ' - ' . __('General configuration', 'woo-osb-payment');
        ...
        $this->title = __('General configuration', 'woo-osb-payment');
        $this->enabled = false;
        ...
        add_action('woocommerce_api_wc_gateway_osb', array($this, 'osb_notify_response'));

        // Filter to allow order status override.
        add_filter('woocommerce_payment_complete_order_status', array($this, 'osb_complete_order_status'), 10, 2);

        // Customize email.
        add_action('woocommerce_email_after_order_table', array($this, 'osb_add_order_email_payment_result'), 10, 3);

        // Print our notices when payment was sucessful.
        add_action('woocommerce_before_template_part', array($this, 'osb_notices'), 10, 4);

        // Delete saved means of payment and saved identifier.
        add_action('woocommerce_api_wc_gateway_osb_delete_saved_card', array($this, 'osb_delete_saved_card'));
    }

    protected function osb_is_section_loaded()
    {
    ...
    protected function osb_init()
    {
        $this->logger = new WC_Logger();

        // Init API.
        $this->osb_request = new OsbRequest();
    ...
    public function osb_admin_head_style()
    {
        ?>
        <style>
            .osb p.description {
            ...
            #woocommerce_osb_url_check + p.description span.url {
                color: #23282d !important;
                font-size: 16px;
                font-weight: bold;
            }

            #woocommerce_osb_url_check + p.description span.desc {
                color: red !important;
            }

            #woocommerce_osb_url_check + p.description img {
                vertical-align: middle;
                margin-right: 5px;
            }

            #woocommerce_osb_rest_check_url + p.description span.url {
                color: #23282d !important;
                font-size: 16px;
                font-weight: bold;
            }

            #woocommerce_osb_rest_check_url + p.description span.desc {
                color: red !important;
                display: inline-block;
            }

            #woocommerce_osb_rest_check_url + p.description img {
                margin-right: 5px;
            }
        </style>
        <?php
    ...
    public function osb_admin_head_script()
    {
        ...
        //<!--
            jQuery(function() {
                osbUpdateCategoryDisplay();
                osbUpdateShippingOptionsDisplay();
            });

            function osbUpdateCategoryDisplay() {
    ...
    public function admin_options()
    {
        if (! $this->is_supported_currency()) {
            echo '<div class="inline error"><p><strong>' . __('Gateway disabled', 'woo-osb-payment') . ': ' . sprintf(__('%s does not support your store currency.', 'woo-osb-payment'), self::GATEWAY_NAME) . '</strong></p></div>';
        }

        if (get_transient($this->id . '_settings_reset')) {
            delete_transient($this->id . '_settings_reset');

            echo '<div class="inline updated"><p><strong>' . sprintf(__('Your %s module configuration is successfully reset.', 'woo-osb-payment'), self::GATEWAY_NAME) . '</strong></p></div>';
        }

        $osb_email_send_msg = get_transient('osb_email_send_msg');
        if ($osb_email_send_msg) {
            echo $osb_email_send_msg;

            delete_transient('osb_email_send_msg');
        }
        ?>

        <script type="text/javascript" src="<?php echo WC_OSB_PLUGIN_URL . '/assets/js/support.js' ?>"></script>
        <br />
        <h3><?php echo self::GATEWAY_NAME; ?></h3>
        <p><?php echo sprintf(__('The module works by sending users to %s in order to select their payment mean and enter their payment information.', 'woo-osb-payment'), self::GATEWAY_NAME); ?></p>
        ...
        <a href="<?php echo $this->reset_admin_link; ?>"><?php _e('Reset configuration', 'woo-osb-payment');?></a>

        <?php
    }

    public function osb_reset_admin_options()
    {
    ...
    protected function get_supported_languages()
    {
        ...
        foreach (OsbApi::getSupportedLanguages() as $code => $label) {
            $langs[$code] = __($label, 'woo-osb-payment');
        }
    ...
    {
        $modes = array(
            '-1' => sprintf(__('%s general configuration', 'woo-osb-payment'), self::GATEWAY_NAME),
            ' ' => sprintf(__('%s Back Office configuration', 'woo-osb-payment'), self::BACKOFFICE_NAME),
            '0' => __('Automatic', 'woo-osb-payment'),
            '1' => __('Manual', 'woo-osb-payment')
        );
    ...
    {
        $categories = array(
            'FOOD_AND_GROCERY' => __('Food and grocery', 'woo-osb-payment'),
            'AUTOMOTIVE' => __('Automotive', 'woo-osb-payment'),
            'ENTERTAINMENT' => __('Entertainment', 'woo-osb-payment'),
            'HOME_AND_GARDEN' => __('Home and garden', 'woo-osb-payment'),
            'HOME_APPLIANCE' => __('Home appliance', 'woo-osb-payment'),
            'AUCTION_AND_GROUP_BUYING' => __('Auction and group buying', 'woo-osb-payment'),
            'FLOWERS_AND_GIFTS' => __('Flowers and gifts', 'woo-osb-payment'),
            'COMPUTER_AND_SOFTWARE' => __('Computer and software', 'woo-osb-payment'),
            'HEALTH_AND_BEAUTY' => __('Health and beauty', 'woo-osb-payment'),
            'SERVICE_FOR_INDIVIDUAL' => __('Service for individual', 'woo-osb-payment'),
            'SERVICE_FOR_BUSINESS' => __('Service for business', 'woo-osb-payment'),
            'SPORTS' => __('Sports', 'woo-osb-payment'),
            'CLOTHING_AND_ACCESSORIES' => __('Clothing and accessories', 'woo-osb-payment'),
            'TRAVEL' => __('Travel', 'woo-osb-payment'),
            'HOME_AUDIO_PHOTO_VIDEO' => __('Home audio, photo, video', 'woo-osb-payment'),
            'TELEPHONY' => __('Telephony', 'woo-osb-payment')
        );
        ...
        if ($no_select_opt) {
            ...
            return array_merge(
                array(
                    '' => '---',
                    'CUSTOM_MAPPING' => __('(Use category mapping below)', 'woo-osb-payment'),
                ),
                $categories
            );
        }
    ...
    {
        return __('Method title to display on payment means page.', 'woo-osb-payment');
    }

    protected function get_method_description_field_description()
    {
        return __('This controls the description which the user sees during checkout.', 'woo-osb-payment');
    }
    ...
    {
        global $woocommerce, $osb_plugin_features;

        // Get log folder.
        if (function_exists('wc_get_log_file_path')) {
            $log_folder = dirname(wc_get_log_file_path('osb')) . '/';
        } else {
        ...
        if (! empty($filenames)) {
            ...
            $docs = __('Click to view the module configuration documentation: ', 'woo-osb-payment');

            foreach ($filenames as $filename) {
                $base_filename = basename($filename, '.pdf');
                $lang = substr($base_filename, -2); // Extract language code.

                $docs .= '<a style="margin-left: 10px; text-decoration: none; text-transform: uppercase;" href="' . WC_OSB_PLUGIN_URL
        ...
        $this->form_fields = array(
            ...
            'module_details' => array(
                'title' => __('MODULE DETAILS', 'woo-osb-payment'),
                'type' => 'title'
            ),
            'developed_by' => array(
                'title' => __('Developed by', 'woo-osb-payment'),
                'type' => 'text',
                'description' => '<b><a href="https://www.lyra.com/" target="_blank">Lyra Network</a></b>',
                'css' => 'display: none;'
            ),
            'contact' => array(
                'title' => __('Contact us', 'woo-osb-payment'),
                'type' => 'text',
                'description' => '<b>' . OsbApi::formatSupportEmails(self::SUPPORT_EMAIL) . '</b>',
                'css' => 'display: none;'
            ),
            'contrib_version' => array(
                'title' => __('Module version', 'woo-osb-payment'),
                'type' => 'text',
                'description' => self::PLUGIN_VERSION,
                'css' => 'display: none;'
            ),
            'platform_version' => array(
                'title' => __('Gateway version', 'woo-osb-payment'),
                'type' => 'text',
            ...
            'base_settings' => array(
                'title' => __('BASE SETTINGS', 'woo-osb-payment'),
                'type' => 'title'
            ),
            'debug' => array(
                'title' => __('Logs', 'woo-osb-payment'),
                'label' => __('Enable / disable', 'woo-osb-payment'),
                'type' => 'checkbox',
                'default' => 'yes',
                'description' => sprintf(__('Enable / disable module logs. The log file will be inside <code>%s</code>.', 'woo-osb-payment'), $log_folder),
            ),

            // Payment gateway access params.
            'payment_gateway_access' => array(
                'title' => __('PAYMENT GATEWAY ACCESS', 'woo-osb-payment'),
                'type' => 'title'
            ),
            'site_id' => array(
                'title' => __('Shop ID', 'woo-osb-payment'),
                'type' => 'text',
                'default' => self::SITE_ID,
                'description' => sprintf(__('The identifier provided by %s.', 'woo-osb-payment'), self::GATEWAY_NAME),
                'custom_attributes' => array('autocomplete' => 'off')
            ),
            'key_test' => array(
                'title' => __('Key in test mode', 'woo-osb-payment'),
                'type' => 'text',
                'default' => self::KEY_TEST,
                'description' => sprintf(__('Key provided by %s for test mode (available in %s Back Office).', 'woo-osb-payment'), self::GATEWAY_NAME, self::BACKOFFICE_NAME),
                'custom_attributes' => array('autocomplete' => 'off')
            ),
            'key_prod' => array(
                'title' => __('Key in production mode', 'woo-osb-payment'),
                'type' => 'text',
                'default' => self::KEY_PROD,
                'description' => sprintf(__('Key provided by %s (available in %s Back Office after enabling production mode).', 'woo-osb-payment'), self::GATEWAY_NAME, self::BACKOFFICE_NAME),
                'custom_attributes' => array('autocomplete' => 'off')
            ),
            'ctx_mode' => array(
                'title' => __('Mode', 'woo-osb-payment'),
                'type' => 'select',
                'default' => self::CTX_MODE,
                'options' => array(
                    'TEST' => __('TEST', 'woo-osb-payment'),
                    'PRODUCTION' => __('PRODUCTION', 'woo-osb-payment')
                ),
                'description' => __('The context mode of this module.', 'woo-osb-payment'),
                'class' => 'wc-enhanced-select'
            ),
            'sign_algo' => array(
                'title' => __('Signature algorithm', 'woo-osb-payment'),
                'type' => 'select',
                'default' => self::SIGN_ALGO,
                'options' => array(
                    OsbApi::ALGO_SHA1 => 'SHA-1',
                    OsbApi::ALGO_SHA256 => 'HMAC-SHA-256'
                ),
                'description' => sprintf(__('Algorithm used to compute the payment form signature. Selected algorithm must be the same as one configured in the %s Back Office.<br /><b>The HMAC-SHA-256 algorithm should not be activated if it is not yet available in the %s Back Office, the feature will be available soon.</b>', 'woo-osb-payment'), self::BACKOFFICE_NAME, self::BACKOFFICE_NAME),
                'class' => 'wc-enhanced-select'
            ),
            'url_check' => array(
                'title' => __('Instant Payment Notification URL', 'woo-osb-payment'),
                'type' => 'text',
                'description' => '<span class="url">' . add_query_arg('wc-api', 'WC_Gateway_Osb', network_home_url('/')) . '</span><br />' .
                    '<img src="' . esc_url(WC_OSB_PLUGIN_URL . 'assets/images/warn.png') . '"><span class="desc">' . sprintf(__('URL to copy into your %s Back Office > Settings > Notification rules.', 'woo-osb-payment'), self::BACKOFFICE_NAME) . '</span>',
                'css' => 'display: none;'
            ),
            'platform_url' => array(
                'title' => __('Payment page URL', 'woo-osb-payment'),
                'type' => 'text',
                'default' => self::GATEWAY_URL,
                'description' => __('Link to the payment page.', 'woo-osb-payment'),
                'css' => 'width: 350px;'
            ),

            // Add REST API key fields.
            'rest_settings' => array(
                'title' => __('REST API keys', 'woo-osb-payment'),
                'type' => 'title',
                'description' => sprintf(__('REST API keys are available in your %s Back Office (menu: Settings > Shops > REST API keys).<br><br>Configure this section if you are using order operations from WooCommerce Back Office or if you are using embedded payment fields or popin modes.', 'woo-osb-payment'), self::BACKOFFICE_NAME)
             ),
             'test_private_key' => array(
                 'title' => __('Test password', 'woo-osb-payment'),
                 'type' => 'password',
                 'default' => '',
                 'custom_attributes' => array('autocomplete' => 'off')
             ),
             'prod_private_key' => array(
                 'title' => __('Production password', 'woo-osb-payment'),
                 'type' => 'password',
                 'default' => '',
                 'custom_attributes' => array('autocomplete' => 'off')
             ),
             'rest_url' => array(
                'title' => __('API REST server URL', 'woo-osb-payment'),
                'type' => 'text',
            ...
            'embedded_fields_keys_settings' => array(
                'title' => '',
                'type' => 'title',
                'description' => sprintf(__('Configure this section only if you are using embedded payment fields or popin modes.', 'woo-osb-payment'), self::BACKOFFICE_NAME)
            ),
             'test_public_key' => array(
                 'title' => __('Public test key', 'woo-osb-payment'),
                 'type' => 'text',
                 'default' => '',
                 'custom_attributes' => array('autocomplete' => 'off')
             ),
             'prod_public_key' => array(
                 'title' => __('Public production key', 'woo-osb-payment'),
                 'type' => 'text',
                 'default' => '',
                 'custom_attributes' => array('autocomplete' => 'off')
             ),
             'test_return_key' => array(
                 'title' => __('HMAC-SHA-256 test key', 'woo-osb-payment'),
                 'type' => 'password',
                 'default' => '',
                 'custom_attributes' => array('autocomplete' => 'off')
             ),
             'prod_return_key' => array(
                 'title' => __('HMAC-SHA-256 production key', 'woo-osb-payment'),
                 'type' => 'password',
                 'default' => '',
                 'custom_attributes' => array('autocomplete' => 'off')
             ),
            'static_url' => array(
                'title' => __('JavaScript client URL', 'woo-osb-payment'),
                'type' => 'text',
                'default' => self::STATIC_URL,
                'css' => 'width: 350px;'
            ),
            'rest_check_url' => array(
                'title' => __('API REST Notification URL', 'woo-osb-payment'),
                'type' => 'text',
                'default' => '',
                'description' => '<span class="url">' . add_query_arg('wc-api', 'WC_Gateway_Osb_Notify_Rest', network_home_url('/')). '</span><br />' .
                '<img src="' . esc_url(WC_OSB_PLUGIN_URL . 'assets/images/warn.png') . '"><span class="desc">' . sprintf(__('URL to copy into your %s Back Office > Settings > Notification rules.<br>In multistore mode, notification URL is the same for all the stores.', 'woo-osb-payment'), self::GATEWAY_NAME). '</span>',
                'css' => 'display: none;'
            ),
            ...
            'language' => array(
                'title' => __('Default language', 'woo-osb-payment'),
                'type' => 'select',
                'default' => self::LANGUAGE,
                'options' => $this->get_supported_languages(),
                'description' => __('Default language on the payment page.', 'woo-osb-payment'),
                'class' => 'wc-enhanced-select'
            ),
            'available_languages' => array(
                'title' => __('Available languages', 'woo-osb-payment'),
                'type' => 'multiselect',
                'default' => array(),
                'options' => $this->get_supported_languages(),
                'description' => __('Languages available on the payment page. If you do not select any, all the supported languages will be available.', 'woo-osb-payment'),
                'class' => 'wc-enhanced-select'
            ),
            'capture_delay' => array(
                ...
                'type' => 'text',
                'default' => '',
                'description' => sprintf(__('The number of days before the bank capture (adjustable in your %s Back Office).', 'woo-osb-payment'), self::BACKOFFICE_NAME)
            ),
            ...
            // Selective 3DS.
            'selective_3ds' => array(
                'title' => __('CUSTOM 3DS', 'woo-osb-payment'),
                'type' => 'title'
            ),
            '3ds_min_amount' => array(
                'title' => __('Manage 3DS', 'woo-osb-payment'),
                'type' => 'text',
                'default' => '',
                'description' => __('Amount below which customer could be exempt from strong authentication. Needs subscription to «Selective 3DS1» or «Frictionless 3DS2» options. For more information, refer to the module documentation.', 'woo-osb-payment')
            ),

            // Return to store params.
            'return_options' => array(
                'title' => __('RETURN OPTIONS', 'woo-osb-payment'),
                'type' => 'title'
            ),
            'redirect_enabled' => array(
                'title' => __('Automatic redirection', 'woo-osb-payment'),
                'label' => __('Enable / disable', 'woo-osb-payment'),
                'type' => 'checkbox',
                'default' => 'no',
                'description' => __('If enabled, the buyer is automatically redirected to your site at the end of the payment.', 'woo-osb-payment')
            ),
            'redirect_success_timeout' => array(
                'title' => __('Redirection timeout on success', 'woo-osb-payment'),
                'type' => 'text',
                'default' => '5',
                'description' => __('Time in seconds (0-300) before the buyer is automatically redirected to your website after a successful payment.', 'woo-osb-payment')
            ),
            'redirect_success_message' => array(
                'title' => __('Redirection message on success', 'woo-osb-payment'),
                'type' => 'multilangtext',
                'default' => array(
                ...
                'description' => __('Message displayed on the payment page prior to redirection after a successful payment.', 'woo-osb-payment'),
                'css' => 'width: 35em;'
            ),
            'redirect_error_timeout' => array(
                'title' => __('Redirection timeout on failure', 'woo-osb-payment'),
                'type' => 'text',
                'default' => '5',
                'description' => __('Time in seconds (0-300) before the buyer is automatically redirected to your website after a declined payment.', 'woo-osb-payment')
            ),
            'redirect_error_message' => array(
                'title' => __('Redirection message on failure', 'woo-osb-payment'),
                'type' => 'multilangtext',
                'default' => array(
                    'en_US' => 'Redirection to shop in a few seconds...',
                    'en_GB' => 'Redirection to shop in a few seconds...',
                ...
                'description' => __('Message displayed on the payment page prior to redirection after a declined payment.', 'woo-osb-payment'),
                'css' => 'width: 35em;'
            ),
            'return_mode' => array(
                'title' => __('Return mode', 'woo-osb-payment'),
                'type' => 'select',
                'default' => 'GET',
                'options' => array(
                    'GET' => 'GET',
                ...
                'description' => __('Method that will be used for transmitting the payment result from the payment page to your shop.', 'woo-osb-payment'),
                'class' => 'wc-enhanced-select'
            ),
            'order_status_on_success' => array(
                'title' => __('Order Status', 'woo-osb-payment'),
                'type' => 'select',
                'default' => 'default',
                'options' => self::get_success_order_statuses(true),
                'description' => __('Defines the status of orders paid with this payment mode.', 'woo-osb-payment'),
                'class' => 'wc-enhanced-select'
            ),

            // Additional options.
            'additional_options' => array(
                'title' => __('ADDITIONAL OPTIONS', 'woo-osb-payment'),
                'type' => 'title',
                'description' => __('Configure this section if you are using advanced risk assessment module or if you are proposing payment means requiring cart data (Franfinance or Klarna).', 'woo-osb-payment')
            ),
            'common_category' => array(
                'custom_attributes' => array(
                    'onchange' => 'osbUpdateCategoryDisplay()'
                ),
                'title' => __('Category mapping', 'woo-osb-payment'),
                'type' => 'select',
                'options' => $this->get_gateway_categories(false),
                'description' => __('Use the same category for all products.', 'woo-osb-payment'),
                'default' => '',
                'class' => 'wc-enhanced-select'
            ),
        );
        ...
        $columns['label'] = array(
            'title' => __('Product category', 'woo-osb-payment'),
            'width' => '154px'
        );

        $columns['category'] = array(
            'title' => sprintf(__('%s category', 'woo-osb-payment'), self::BACKOFFICE_NAME),
            'width' => '154px'
        );
        ...
        $this->form_fields['category_mapping'] = array(
            ...
            'columns' => $columns,
            'description' => sprintf(__('Match each product category with a %s product category. <br /><b>Entries marked with * are newly added and must be configured.</b>', 'woo-osb-payment'), self::BACKOFFICE_NAME)
        );
        ...
        $columns['method_title'] = array(
            'title' => __('Method title', 'woo-osb-payment'),
            'width' => '210px'
        );

        $columns['type'] = array(
            'title' => __('Type', 'woo-osb-payment'),
            'width' => '130px'
        );

        $columns['speed'] = array(
            'title' => __('Rapidity', 'woo-osb-payment'),
            'width' => '75px',
        );

        $columns['delay'] = array(
            'title' => __('Delay', 'woo-osb-payment'),
            'width' => '90px',
        );

        $this->form_fields['shipping_options'] = array(
            'title' => __('Shipping options', 'woo-osb-payment'),
            'type' => 'shipping_table',
            'columns' => $columns,
            'description' => $descr
        );

        if ($osb_plugin_features['qualif']) {
        ...
        if (! $osb_plugin_features['support']) {
    ...
    {
        $this->general_settings = get_option('woocommerce_osb_settings', null);
    ...
    public function generate_support_component_html($key, $data)
    {
        ...
        $send_email_url = add_query_arg('wc-api', 'WC_Gateway_Osb_Send_Email', home_url('/'));
        ...
        <tr valign="top">
            <th scope="row" class="titledesc">
                <contact-support
                    ...
                    contrib="<?php echo OsbTools::get_contrib(); ?>"
                    integration-mode="<?php echo OsbTools::get_integration_mode(); ?>"
                    plugins="<?php echo OsbTools::get_active_plugins(); ?>"
                    title=""
                    ...
                    cc-emails=""
                    phone-number=""
                    language="<?php echo OsbTools::get_support_component_language(); ?>"></contact-support>
            </th>
        </tr>
    ...
    public function generate_table_html($key, $data)
    {
        ...
        $html .= '<input id="' . $field_name . '_btn" class="' . $field_name . '_btn '. esc_attr($data['class']) . '"' . (! empty($options) ? ' style="display: none;"' : '') . ' type="button" value="' . __('Add', 'woo-osb-payment') . '">';
        ...
        $html .= "\n" . 'jQuery(".' . $field_name . '_btn").click(function() {
                            osbAddOption("' . $field_name . '", ' . json_encode($record) . ');
                         })';

        if (! empty($options)) {
            // Add already inserted lines.
            foreach ($options as $code => $option) {
                $html .= "\n" . 'osbAddOption("' . $field_name . '", ' . json_encode($option) . ', "' . $code . '");';
            }
        }
    ...
    public function generate_shipping_table_html($key, $data)
    {
        ...
        $shipping_types = array(
            'PACKAGE_DELIVERY_COMPANY' => __('Delivery company', 'woo-osb-payment'),
            'RECLAIM_IN_SHOP' => __('Reclaim in shop', 'woo-osb-payment'),
            'RELAY_POINT' => __('Relay point', 'woo-osb-payment'),
            'RECLAIM_IN_STATION' => __('Reclaim in station', 'woo-osb-payment')
        );

        $shipping_rapidities = array(
            'STANDARD' => __('Standard', 'woo-osb-payment'),
            'EXPRESS' => __('Express', 'woo-osb-payment'),
            'PRIORITY' => __('Priority', 'woo-osb-payment')
        );

        $shipping_delays = array(
            'INFERIOR_EQUALS' => __('<= 1 hour', 'woo-osb-payment'),
            'SUPERIOR' => __('> 1 hour', 'woo-osb-payment'),
            'IMMEDIATE' => __('Immediate', 'woo-osb-payment'),
            'ALWAYS' =>  __('24/7', 'woo-osb-payment'),
        );
        ...
        foreach ($options as $code => $option) {
            ...
            $html .= '<td style="padding: 5px;"><select class="osb_list_speed" id="' . $field_name . '_' . $code . '_speed"
                      name="' . $field_name . '[' . $code . '][speed]"
                      value="' . (isset($option['speed']) ? $option['speed'] : '') . '"
                      onchange="javascript:osbUpdateShippingOptionsDisplay(\'' . $field_name . '_' . $code . '_speed\')">';
    ...
    public function validate_ctx_mode_field($key, $value = null)
    {
        ...
        if (! $new_value && $osb_plugin_features['qualif']) {
     ...
     **/
    public function osb_notify_response()
    {
        global $woocommerce;
        ...
        $from_server = $osb_response->get('hash') != null;

        if (! $osb_response->isAuthentified()) {
            ...
            if ($from_server) {
                ...
                die($osb_response->getOutputForGateway('auth_fail'));
            } else {
                ...
                $iframe = $osb_response->get('action_mode') === 'IFRAME';
                $this->osb_redirect($cart_url, $iframe);
            }
        } else {
            header('HTTP/1.1 200 OK');

            $this->osb_manage_notify_response($osb_response);
        }
    }
     ...
     **/
    public function osb_manage_notify_response($osb_response, $from_server_rest = false)
    {
        global $woocommerce, $osb_plugin_features;
        ...
        $order_id = $osb_response->get('order_id');
        $from_server = $osb_response->get('hash') != null || $from_server_rest;
        $iframe = $osb_response->get('action_mode') == 'IFRAME';
        ...
        $key = $osb_response->getExtInfo('order_key');
        ...
        if (! self::get_order_property($order, 'id') || ($key && (self::get_order_property($order, 'order_key') !== $key))) {
            ...
            if ($from_server) {
                $this->log('IPN URL PROCESS END');
                die($osb_response->getOutputForGateway('order_not_found'));
            } else {
                // Fatal error, empty cart.
                $woocommerce->cart->empty_cart();
                $this->add_notice(__('An error has occurred in the payment process.', 'woo-osb-payment'), 'error');
        ...
        if (! $from_server && $this->testmode && $osb_plugin_features['prodfaq']) {
            $msg = __('<p><u>GOING INTO PRODUCTION</u></p>You want to know how to put your shop into production mode, please read chapters « Proceeding to test phase » and « Shifting the shop to production mode » in the documentation of the module.', 'woo-osb-payment');
        ...
        if ($this->is_new_order($order, $osb_response->get('trans_id'), $osb_response->get('sequence_number'))) {
            // Order not processed yet or a failed payment (re-order).

            // Add order note.
            self::osb_add_order_note($osb_response, $order);
            ...
            update_post_meta((int) $order_id, 'Transaction ID', $osb_response->get('trans_id'));
            update_post_meta((int) $order_id, 'Card number', $osb_response->get('card_number'));
            update_post_meta((int) $order_id, 'Means of payment', $osb_response->get('card_brand'));
            update_post_meta((int) $order_id, 'Sequence number', $osb_response->get('sequence_number'));

            // Store authorized amount.
            if ($authorized_amount = $osb_response->get('authorized_amount')) {
                delete_post_meta((int) $order_id, 'Authorized amount');
                update_post_meta((int) $order_id, 'Authorized amount', self::display_amount($authorized_amount, $osb_response->get('currency')));
            }

            // Store installments number/config.
            if (($installments_number = $osb_response->get('payment_option_code')) && is_numeric($installments_number)) {
                delete_post_meta((int) $order_id, 'Installments number');
                update_post_meta((int) $order_id, 'Installments number', $installments_number);
            }

            $expiry = '';
            if ($osb_response->get('expiry_month') && $osb_response->get('expiry_year')) {
                $expiry = str_pad($osb_response->get('expiry_month'), 2, '0', STR_PAD_LEFT) . '/' . $osb_response->get('expiry_year');
            }

            update_post_meta((int) $order_id, 'Card expiry', $expiry);

            if (self::is_successful_action($osb_response)) {
                if ($osb_response->isPendingPayment()) {
                    // Payment is pending.
                ...
                $this->osb_save_identifier($order, $osb_response);

                if ($subscriptions_handler) {
                    // Try to save subscritption info if any.
                    $this->osb_save_recurrence($order, $osb_response);

                    $subscriptions_handler->process_subscription($order, $osb_response);
                }
                ...
                if ($from_server) {
                    ...
                    die($osb_response->getOutputForGateway('payment_ok'));
                } else {
                    $this->log("Warning! IPN URL call has not worked. Payment completed by return URL call for order #$order_id.");

                    if ($this->testmode) {
                        $ipn_url_warn = sprintf(__('The automatic validation has not worked. Have you correctly set up the notification URL in the %s Back Office?', 'woo-osb-payment'), self::BACKOFFICE_NAME);
                        $ipn_url_warn .= '<br />';
                        $ipn_url_warn .= __('For understanding the problem, please read the documentation of the module : <br />&nbsp;&nbsp;&nbsp;- Chapter &laquo; To read carefully before going further &raquo;<br />&nbsp;&nbsp;&nbsp;- Chapter &laquo; Notification URL settings &raquo;', 'woo-osb-payment');

                        $this->add_notice($ipn_url_warn, 'error');
                    }

                    $this->log('RETURN URL PROCESS END');
                    $this->osb_redirect($this->get_return_url($order), $iframe);
                }
            } else {
                $order->update_status('failed');
                $this->log("Payment failed or cancelled for order #$order_id. {$osb_response->getLogMessage()}");
                ...
                if ($from_server) {
                    $this->log('IPN URL PROCESS END');
                    die($osb_response->getOutputForGateway('payment_ko'));
                } else {
                    $this->log('RETURN URL PROCESS END');

                    if (! $osb_response->isCancelledPayment()) {
                        $this->add_notice(__('Your payment was not accepted. Please, try to re-order.', 'woo-osb-payment'), 'error');
                        $this->osb_redirect($cart_url, $iframe);
                    } else {
                        $this->osb_redirect($checkout_url, $iframe);
                    }
                }
            }
        } else {
            ...
            if ($osb_response->get('recurrence_number') && $subscriptions_handler) {
                ...
                $subscriptions_handler->process_subscription_renewal($order, $osb_response);

                if (self::is_successful_action($osb_response)) {
                    $this->log("Payment recurrence processed successfully by IPN URL call for order #$order_id.");
                    echo($osb_response->getOutputForGateway('payment_ok'));
                } else {
                    echo($osb_response->getOutputForGateway('payment_ko'));
                }

                $this->log('IPN URL PROCESS END');
                die();
            } elseif (($osb_response->get('identifier_status') === 'UPDATED')
                /* Case of method change from WooCommerce or subscription creation on gateway Back Office. */
                || ((($osb_response->get('recurrence_status') === 'CREATED') || ($osb_response->get('recurrence_number') === '1'))
                        && ($subscriptions_handler = self::subscriptions_handler('osbsubscription')))) {
                ...
                if ($subsc_id = $osb_response->getExtInfo('subsc_id')) {
                ...
                if (self::is_successful_action($osb_response)) {
                    ...
                    $force_update = $osb_response->get('recurrence_number') === '1';

                    if ($force_update && ($method !== 'osbsubscription')) {
                        // Update payment method in order.
                        $osb_subscription = new WC_Gateway_OsbSubscription();
                        if (method_exists($order, 'set_payment_method')) {
                            $order->set_payment_method($osb_subscription);
                        } else {
                            $order->payment_method = $osb_subscription->id;
                            $order->payment_method_title = $osb_subscription->get_title();
                        }
                    ...
                    // Try to save identifier if any.
                    $this->osb_save_identifier($order, $osb_response, $force_update);

                    // Try to save subscritption info if any.
                    $this->osb_save_recurrence($order, $osb_response, $force_update);
                    ...
                    if ($from_server) {
                        $this->log('IPN URL PROCESS END');
                        die($osb_response->getOutputForGateway('payment_ok_already_done'));
                    } else {
                        $this->add_notice(__('Payment method updated.', 'woocommerce-subscriptions'));

                        $this->log('RETURN URL PROCESS END');
                        $this->osb_redirect($subsc_redirect_url ? $subsc_redirect_url : $this->get_return_url($order), $iframe);
                    }
                } else {
                    if ($from_server) {
                        $this->log('IPN URL PROCESS END');
                        ...
                        if (! $osb_response->isCancelledPayment()) {
                            ...
                            $this->osb_redirect($subsc_redirect_url ? $subsc_redirect_url : $cart_url, $iframe);
                        } else {
                            $this->osb_redirect($subsc_redirect_url ? $subsc_redirect_url : $checkout_url, $iframe);
                        }
                    }
                }
            }
            ...
            if ($from_server && (self::get_order_property($order, 'status') === 'on-hold')) {
                switch (true) {
                    case $osb_response->isCancelledPayment():
                        $this->log("Order #$order_id is in a pending status and payment is cancelled. It may be a payment expiration. Do nothing.");
                        echo($osb_response->getOutputForGateway('payment_ko_already_done'));
                        break;
                    case $osb_response->isPendingPayment():
                        $this->log("Order #$order_id is in a pending status and stays in the same status. Do nothing.");
                        echo($osb_response->getOutputForGateway('payment_ok_already_done'));
                        break;
                    case self::is_successful_action($osb_response):
                ...
                $this->log('IPN URL PROCESS END');
                die();
            } elseif (self::is_successful_action($osb_response) && key_exists(self::get_order_property($order, 'status'), self::get_success_order_statuses(false, $subscriptions_handler))) {
                $status = $osb_response->isPendingPayment() ? 'pending' : 'successful';
            ...
            } elseif (! self::is_successful_action($osb_response) && (self::get_order_property($order, 'status') === 'failed' || self::get_order_property($order, 'status') === 'cancelled')) {
                $this->log("Payment failed confirmed for order #$order_id.");

                // Order failure registered and payment error received.
                if ($from_server) {
                    $this->log('IPN URL PROCESS END');
                    die($osb_response->getOutputForGateway('payment_ko_already_done'));
                } else {
                    $this->log('RETURN URL PROCESS END');
                ...
                $this->log("Error! Invalid payment result received for already saved order #$order_id. Payment result : {$osb_response->getTransStatus()}, Order status : {self::get_order_property($order, 'status')}.");

                // Registered order status not match payment result.
                if ($from_server) {
                    $this->log('IPN URL PROCESS END');
                    die($osb_response->getOutputForGateway('payment_ko_on_order_ok'));
                } else {
                    // Fatal error, empty cart.
                    $woocommerce->cart->empty_cart();
                    $this->add_notice(__('An error has occurred in the payment process.', 'woo-osb-payment'), 'error');
    ...
    public function osb_complete_order_status($status, $order_id)
    {
        $order = new WC_Order((int)$order_id);

        if ((strpos(self::get_order_property($order, 'payment_method'), 'osb') === 0)
    ...
    {
        if ($order->has_status(apply_filters('woocommerce_osb_valid_order_statuses', array('pending'), $order))) {
            return true;
        }
    ...
    public function osb_add_order_email_payment_result($order, $sent_to_admin, $plain_text = false)
    {
        if (strpos(self::get_order_property($order, 'payment_method'), 'osb') !== 0) {
            return;
        }
        ...
        if (isset($osb_order_note)) {
            if ($plain_text) {
                echo strtoupper(__('Payment', 'woo-osb-payment')) . "\n\n" . $osb_order_note . "\n\n";
            } else {
                echo '<h2>' . __('Payment', 'woo-osb-payment') . '</h2><p>' . str_replace("\n", '<br >', $osb_order_note) . '</p>';
            }
        }
    }
    ...
    protected function osb_redirect($url, $iframe = false)
    {
        if (! $iframe) {
            wp_redirect($url);
        } else {
    ...
    public static function osb_add_order_note($osb_response, $order)
    {
        $note = $osb_response->getCompleteMessage("\n");

        if ($osb_response->get('brand_management')) {
            $brand_info = json_decode($osb_response->get('brand_management'));
            $msg_brand_choice = "\n";

            if (isset($brand_info->userChoice) && $brand_info->userChoice) {
                $msg_brand_choice .= __('Card brand chosen by buyer.', 'woo-osb-payment');
            } else {
                $msg_brand_choice .= __('Default card brand used.', 'woo-osb-payment');
            }
        ...
        // 3DS extra message.
        $note = __('3DS authentication: ', 'woo-osb-payment');
        if ($status = $osb_response->get('threeds_status')) {
            $note .= self::get_threeds_status($status);

            if ($threeds_cavv = $osb_response->get('threeds_cavv')) {
                $note .= "\n";
                $note .= __('3DS certificate: ', 'woo-osb-payment') . $threeds_cavv;
            }

            if ($threeds_auth_type = $osb_response->get('threeds_auth_type')) {
                $note .= "\n";
                $note .= __('Authentication type: ', 'woo-osb-payment') . $threeds_auth_type;
            }
        } else {
            $note .= 'UNAVAILABLE';
        }
        ...
        $note = '';
        if (! $osb_response->isCancelledPayment()) {
            $note .= sprintf(__('Transaction ID: %s.', 'woo-osb-payment'), $osb_response->get('trans_id'));

            if ($osb_response->get('trans_uuid')) {
                $note .= "\n";
                $note .= sprintf(__('Transaction UUID: %s.', 'woo-osb-payment'), $osb_response->get('trans_uuid'));
            }
        }

        if ($osb_response->getTransStatus()) {
            $note .= "\n";
            $note .= sprintf(__('Transaction status: %s.', 'woo-osb-payment'), $osb_response->getTransStatus());
        }
    ...
    public function osb_notices($template_name, $template_path, $located, $args = array())
    {
        global $woocommerce;
        ...
        if (strpos(self::get_order_property($args['order'], 'payment_method'), 'osb') === 0) {
    ...
    private static function get_success_order_statuses($default = false, $subscriptions_handler = false)
    {
        ...
        if (function_exists('wc_get_order_statuses')) { // From WooCommerce 2.2.0.
            ...
            $statuses = array(
                'on-hold' => __('On Hold', 'woo-osb-payment'),
                'processing' => __('Processing', 'woo-osb-payment'),
                'completed' => __('Complete', 'woo-osb-payment')
            );
        }

        if ($default) {
            $statuses = array('default' => __('Default', 'woo-osb-payment')) + $statuses;
        }
    ...
    public static function is_successful_action($osb_response)
    {
        if ($osb_response->isAcceptedPayment()) {
            return true;
        }
        ...
        if ($osb_response->get('subscription') && ($osb_response->get('recurrence_status') === 'CREATED')) {
            return true;
        }

        if ($osb_response->get('identifier') && (
            $osb_response->get('identifier_status') == 'CREATED' /* page_action is REGISTER_PAY or ASK_REGISTER_PAY */ ||
            $osb_response->get('identifier_status') == 'UPDATED' /* page_action is REGISTER_UPDATE_PAY */
        )) {
            return true;
        }
    ...
    private function osb_save_identifier($order, $osb_response, $force_update = false)
    {
        ...
        if ($osb_response->get('identifier') && ($force_update || (
            $osb_response->get('identifier_status') == 'CREATED' /* page_action is REGISTER_PAY or ASK_REGISTER_PAY */ ||
            $osb_response->get('identifier_status') == 'UPDATED' /* page_action is REGISTER_UPDATE_PAY */
        ))) {
            ...
            $identifier_info = array(
                'identifier' => $osb_response->get('identifier'),
                'active' => true
            );
            ...
            $number = $osb_response->get('card_number');
            $masked = '';
            ...
            if (preg_match('#^([A-Z]{2}[0-9]{2}[A-Z0-9]{10,30})(_[A-Z0-9]{8,11})?$#i', $number, $matches)) {
                ...
                $masked = $osb_response->get('card_brand') . '|' .  str_repeat('X', strlen($number) - 4) . substr($number, - 4);

                if ($osb_response->get('expiry_month') && $osb_response->get('expiry_year')) {
                    // Format card expiration data.
                    $masked .= ' (';
                    $masked .= str_pad($osb_response->get('expiry_month'), 2, '0', STR_PAD_LEFT);
                    $masked .= '/';
                    $masked .= $osb_response->get('expiry_year');
                    $masked .= ')';
                }
            }
    ...
    private function osb_save_recurrence($order, $osb_response, $force_update = false)
    {
        $order_id = (int) self::get_order_property($order, 'id');

        if ($osb_response->get('subscription') && ($force_update || $osb_response->get('recurrence_status') === 'CREATED')) {
            $this->log("Subscription for order #{$order_id} successfully created on payment gateway. Let's save subscription information.");

            $currency_code = $osb_response->get('sub_currency');
            ...
            update_post_meta($order_id, 'Subscription ID', $osb_response->get('subscription'));
            update_post_meta($order_id, 'Subscription amount', self::display_amount($osb_response->get('sub_amount'), $currency_code));
            update_post_meta($order_id, 'Effect date', preg_replace('#^(\d{4})(\d{2})(\d{2})$#', '\1-\2-\3', $osb_response->get('sub_effect_date')));

            if ($osb_response->get('sub_init_amount')) {
                update_post_meta($order_id, 'Initial amount', self::display_amount($osb_response->get('sub_init_amount'), $currency_code));
                update_post_meta($order_id, 'Initial amount count', $osb_response->get('sub_init_amount_number'));
            }
    ...
    public static function display_amount($amount_in_cents, $currency_code)
    {
        ...
        $currency = OsbApi::findCurrencyByNumCode($currency_code);
    ...
    private static function subscriptions_handler($method)
    {
        ...
        return Osb_Subscriptions_Loader::getInstance($handler);
    }

    public function osb_delete_saved_card()
    {
        global $woocommerce;
        ...
        if (is_user_logged_in() && isset($_POST['id']) && ! empty($_POST['id'])) {
            ...
            try {
                ...
                if ($key) {
                    ...
                    OsbRestTools::checkResult($result);
                ...
                if (in_array($e->getCode(), $invalid_ident_codes)) {
                    ...
                    $this->add_notice(__('The stored means of payment could not be deleted.', 'woo-osb-payment'), 'error');
                    return false;
                }
            }
        }
    ...
    private function delete_identifier_attributes($cust_id, $id)
    {
        ...
        $this->add_notice(__('The stored means of payment was successfully deleted.', 'woo-osb-payment'));
    }
    ...
    protected function check_identifier($cust_id, $id)
    {
        ...
        try {
            ...
            $cancellation_date = OsbRestTools::getProperty($result['answer'], 'cancellationDate');
